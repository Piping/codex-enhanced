...
same private context. One-shot loops create a temporary hidden thread, run once, and are removed
after completion.
...
One-shot loops start a fresh hidden thread from compact main-thread context.
...
- No per-loop custom sandbox/tool policy
