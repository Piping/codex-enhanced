...
use crate::bottom_pane::discover_prompts_in;
...
pub(crate) struct ChatWidget {
    ...
    // Last elapsed-seconds value emitted on a work separator, used as the baseline for the next one.
...
impl ChatWidget {
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        let prompts_dir = widget.config.codex_home.join("prompts");
        widget
            .bottom_pane
            .set_custom_prompts(discover_prompts_in(&prompts_dir));
    ...
    /// Request a respawn or in-process reattach, depending on the active thread context.
