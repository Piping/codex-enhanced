...
pub(crate) struct PersistedLoopExecutionSettings {
    #[serde(default)]
...
pub(crate) fn apply_loop_execution_settings(
    config: &mut Config,
...
pub(crate) fn loop_execution_summary(
    settings: &PersistedLoopExecutionSettings,
    session_cwd: &Path,
) -> String {
    let cwd = settings
...
pub(crate) fn writable_roots_editor_text(settings: &PersistedLoopExecutionSettings) -> String {
    settings
        .writable_roots
        .iter()
...
pub(crate) fn parse_loop_writable_roots(
    input: &str,
    workspace_cwd: &Path,
...
pub(crate) fn parse_loop_cwd(input: &str, workspace_cwd: &Path) -> Result<PathBuf, String> {
