...
impl App {
    ...
    async fn handle_event(
        ...
        let event = match self.dispatch_feature_event(tui, app_server, event).await? {
            feature_dispatch::FeatureDispatchOutcome::Unhandled(event) => *event,
            feature_dispatch::FeatureDispatchOutcome::Handled => {
                return Ok(AppRunControl::Continue);
            }
            feature_dispatch::FeatureDispatchOutcome::Return(control) => {
                return Ok(control);
            }
        };
