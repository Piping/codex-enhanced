...
mod tests {
    ...
    #[test]
    fn persist_managed_account_round_trips() {
        let tempdir = tempdir().expect("tempdir");
        let store = ManagedAccountAuthStore::new(tempdir.path().to_path_buf());
        let auth = chatgpt_auth("account-1", "person@example.com");

        persist_managed_account_auth_snapshot(tempdir.path(), &auth, Some("primary"))
            .expect("persist snapshot");

        let loaded = store.load_account_auth("account-1").expect("load auth");
        assert_eq!(loaded, auth);
    }

    #[test]
    fn persist_current_managed_account_snapshot_saves_active_auth() {
        let tempdir = tempdir().expect("tempdir");
        let auth = chatgpt_auth("account-1", "person@example.com");
        save_auth(tempdir.path(), &auth, AuthCredentialsStoreMode::Plaintext).expect("save auth");
        ...
        assert_eq!(snapshot.profile.id, "account-1");
    }

    #[test]
    fn activate_managed_account_restores_saved_auth() {
        let tempdir = tempdir().expect("tempdir");
        let store = ManagedAccountAuthStore::new(tempdir.path().to_path_buf());
        let auth = chatgpt_auth("account-1", "person@example.com");
        store
            .save_account_auth("account-1", &auth)
            .expect("save account auth");
        ...
        let active = load_auth_dot_json(tempdir.path(), AuthCredentialsStoreMode::Plaintext)
            .expect("load auth")
            .expect("active auth");
        assert_eq!(active, auth);
    }
}
