...
- Before PTY verification with the local binary, run `bash install_local.sh` from `codex-rs/` so the debug binary is signed and installed over `/usr/local/bin/codex`.
