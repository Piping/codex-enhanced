...
- Install any commands the repo relies on (for example `just`, `rg`, `ast-grep`, or `cargo-insta`) if they aren't already available before running instructions here.
- Prefer `ast-grep` for Rust structure-aware search when you need syntax-level accuracy instead of plain text matching.
  - Good fits: `match`, `if let`, `impl Trait for Type`, attribute macros, and specific call shapes.
  - Use `rg` for exact literals, file names, and quick broad-text scans; use `ast-grep` when `rg` would overmatch nearby text or miss multiline syntax.
- Repo-local skills must live under `.agents/skills/`. Do not add or keep repo-specific skills under `.codex/skills/`; treat `.agents/` as the only repo-local home for skills and related agent assets.
...
- 在跑 codex 命令之前, 总是先执行 `install target/debug/codex ~/.local/bin/` 来完成本地 debug 二进制覆盖安装, 然后再运行 `codex` 或做 PTY 验证
- 当需求改变的时候, 要同步更新对应的文档
